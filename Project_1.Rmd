...
Looking at EDA results, I can assume that these variables could correlate with each other. To check that, I used Spearman correlation method.

```{r correlation test}
cor.test(mussels$Diameter, mussels$Whole_weight, method = 'spearman')
```

Indeed, according to Spearman's correlation, Diameter and Whole_weight demonstrate strong positive statistically significant correlation between them. It can be visualized with a plot like this:
